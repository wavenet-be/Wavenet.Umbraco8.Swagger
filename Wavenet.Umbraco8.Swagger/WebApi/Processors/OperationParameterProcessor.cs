// <copyright file="OperationParameterProcessor.cs" company="Wavenet">
...
namespace Wavenet.Umbraco8.Swagger.WebApi.Processors
{
    ...
    internal class OperationParameterProcessor : IOperationProcessor
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OperationParameterProcessor" /> class.
        /// </summary>
        ...
        public bool Process(OperationProcessorContext context)
        {
                            ...
                            !p.ContextAttributes.GetAssignableToTypeName("BindNeverAttribute", TypeNameStyle.Name).Any()))
            {
                    ...
                    lowerHttpPath.Contains("{" + uriParameterNameLower + ":") /*path parameter*/)
                {
                    ...
                    if (this.settings.SchemaType == SchemaType.Swagger2)
                    {
                ...
                {
                    var parameterInfo = this.settings.ReflectionService.GetDescription(contextualParameter, this.settings);
                    operationParameter = this.TryAddFileParameter(context, parameterInfo, contextualParameter);
                    if (operationParameter == null)
                    {
                        ...
                        else
                        {
                            if (parameterInfo.IsComplexType)
                            {
                                ...
                                if (parameterBindingAttribute != null && fromBodyAttribute == null && fromUriAttribute == null && !this.settings.IsAspNetCore)
                                {
                                    ...
                                    {
                                        operationParameter = this.AddBodyParameter(context, bodyParameterName, contextualParameter);
                                    }
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            operationParameter = this.AddPrimitiveParameter(uriParameterName, context, contextualParameter);
                                        }
                                    }
                                }
                                else if (fromBodyAttribute != null || (fromUriAttribute == null && !this.settings.IsAspNetCore))
                                {
                                    operationParameter = this.AddBodyParameter(context, bodyParameterName, contextualParameter);
                                }
                                else
                                {
                                    operationParameter = this.AddPrimitiveParametersFromUri(context, httpPath, uriParameterName, contextualParameter, parameterInfo);
                                }
                            }
                            else
                            {
                ...
                if (operationParameter != null)
                {
                    ...
                    if (this.settings.SchemaType == SchemaType.OpenApi3)
                    {
            ...
            if (this.settings.AddMissingPathParameters)
            {
            ...
            this.RemoveUnusedPathParameters(context.OperationDescription, httpPath);
            this.UpdateConsumedTypes(context.OperationDescription);

            this.EnsureSingleBodyParameter(context.OperationDescription);
        ...
        private OpenApiParameter AddBodyParameter(OperationProcessorContext context, string name, ContextualParameterInfo contextualParameter)
        {
            ...
            var typeDescription = this.settings.ReflectionService.GetDescription(contextualParameter, this.settings);
            var isNullable = this.settings.AllowNullableBodyParameters && typeDescription.IsNullable;
        ...
        private OpenApiParameter AddFileParameter(OperationProcessorContext context, ContextualParameterInfo contextualParameter, bool isFileArray)
        {
            ...
            this.InitializeFileParameter(operationParameter, isFileArray);
            context.OperationDescription.Operation.Parameters.Add(operationParameter);
            return operationParameter;
        }
            ...
            else
            {
                foreach (var contextualProperty in contextualParameter.Type.GetContextualProperties())
                {
                        ...
                        !a.IsAssignableToTypeName("JsonIgnoreAttribute", TypeNameStyle.Name)))
                    {
                        ...
                        var parameterInfo = this.settings.ReflectionService.GetDescription(contextualProperty, this.settings);
                        var isFileArray = this.IsFileArray(contextualProperty.Type, parameterInfo);
        ...
        private bool IsFileArray(Type type, JsonTypeDescription typeInfo)
        {
            ...
            {
                var description = this.settings.ReflectionService.GetDescription(type.GenericTypeArguments[0].ToContextualType(), this.settings);
        ...
        {
            var isFileArray = this.IsFileArray(contextualParameter.Type, typeInfo);
            ...
            if (typeInfo.Type == JsonObjectType.File ||
                ...
                isFileArray)
            {
                return this.AddFileParameter(context, contextualParameter, isFileArray);
            }
